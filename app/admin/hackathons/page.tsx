...
export default function HackathonsAdmin() {
    ...
    const [formData, setFormData] = useState({ name: '', description: '', role: '', year: new Date().getFullYear() });
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await fetch(`/api/admin/hackathons?id=${id}`, { method: 'DELETE' });
            fetchHackathons();
        } catch (error) {
            console.error(error);
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setIsAdding(false);
            setEditingId(null);
            setFormData({ name: '', description: '', role: '', year: new Date().getFullYear() });
            fetchHackathons();
        } catch (error) {
            console.error(error);
        }
        ...
        <div className="flex flex-col gap-8">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-gray-400">Add, edit, or delete your hackathon experiences.</p>
                </div>
                <button
                    onClick={() => {
                        if (isAdding) {
                ...
                >
                    <FaPlus /> {isAdding ? 'Cancel' : 'Add Hackathon'}
                </button>
            </div>

            {isAdding && (
                <div className="glass p-6 rounded-2xl">
                    <h2 className="text-xl font-bold mb-4">{editingId ? 'Edit Hackathon' : 'Add Hackathon'}</h2>
                    <form onSubmit={handleSubmit} className="flex flex-col gap-4 max-w-lg">
                        ...
                        <div>
                            ...
                            <input type="text" required value={formData.role} onChange={e => setFormData({ ...formData, role: e.target.value })} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-2 text-white outline-none" />
                        </div>
                        <div>
                            <label className="text-sm text-gray-400 block mb-1">Description</label>
                            <input type="text" required value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-2 text-white outline-none" />
                        </div>
                        <div>
                        ...
                        <div className="flex gap-4 mt-2">
                            <button type="submit" className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-xl w-fit transition-colors">
                                {editingId ? 'Update Hackathon' : 'Save Hackathon'}
                            </button>
                            {editingId && (
                    ...
                    <tbody>
                        {hackathons.map((h) => (
                            <tr key={h.id} className="border-b border-white/5 hover:bg-white/5">
                                ...
                                <td className="p-4 text-primary">{h.role}</td>
                                <td className="p-4 text-gray-400">{h.year}</td>
                                <td className="p-4 flex justify-end gap-3">
                                    <button onClick={() => handleEdit(h)} className="text-blue-400 hover:text-blue-300 p-2 transition-colors"><FaEdit /></button>
                                    <button onClick={() => handleDelete(h.id)} className="text-red-400 hover:text-red-300 p-2 transition-colors"><FaTrash /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
